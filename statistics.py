...
# Generates A CSV file with the data along with a graph to visualise the data.
...
def gym_analysis(venue, start_date, end_date):
    ...
    try:
        ...
        df = pd.read_csv("combined_ledger.csv", names=cols)  # Reads the ledger and stores it in a dataframe
        ...
        tk.messagebox.showerror(message="No Data Found") # If the ledger is not found, an error message is displayed
        exit()

    start_time = 0
    end_time = 24

    if start_date is None and end_date is None: # If the user selects all data, the start_date and end_date are set to None
        # Filters through the dataframe to find the data that matches the venue, time range, and status.
        if venue == "ALL":
        ...
        try:
            ...
            tkinter.messagebox.showerror(message="Please Select a different date range", title="Error", icon="error") # If the user selects a date range that is not in the ledger, an error message is displayed
            exit()
    try:
        ...
        print(f"Venue: {venue}\nBottom Quartile: {bottom_quartile}\nAverage: {average}\nTop Quartile: {top_quartile}")
            ...
            tk.messagebox.showerror(message="No data available for this date range", title="Error", icon="error")  # Throws an error if the user selects a date range that is not in the ledger
            exit()


    ax = sns.lineplot(data=data, x="time", y="count", hue="venue", errorbar=None) # Plots the data
    ...
    plt.savefig("graph.png", pad_inches=0.5) # Saves the graph as a png file
    plt.show()


    data = data.groupby(["time", "venue"]).mean(numeric_only=True).round(0).reset_index().sort_values(by="count") # Groups the data by time and venue and calculates the average
    data.to_csv("data.csv", index=False) # Saves the data as a csv file

gym_analysis("ALL", None, None)
